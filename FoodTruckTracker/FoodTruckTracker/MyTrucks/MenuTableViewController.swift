...
class MenuTableViewController: UITableViewController {
    ...
    private lazy var fetchedResultsController: NSFetchedResultsController<MenuItem> = {
        let request: NSFetchRequest<MenuItem> = MenuItem.fetchRequest()
        request.sortDescriptors = [
//            .init(key: "<#sortKey#>", ascending: true),
//            .init(key: "<#anotherSortKey#>", ascending: true, selector: #selector(NSString.localizedStandardCompare(_:))),
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard  let cell = tableView.dequeueReusableCell(withIdentifier: "FoodCell", for: indexPath) as? FoodTableViewCell else {
            fatalError("Can't dequeue cell of type \(FoodTableViewCell.reuseIdentifier)")
        }
        cell.menuItem = fetchedResultsController.object(at: indexPath)
